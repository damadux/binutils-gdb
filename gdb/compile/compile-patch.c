...
/* This function aims to find the next available chunk of memory around insn_address
...
static CORE_ADDR
allocate_trampoline(gdbarch *gdbarch, CORE_ADDR insn_address, int trampoline_size)
{
  ...
  CORE_ADDR trampoline_address = ((insn_address>>bit_shift) > 0 ? (((insn_address>>bit_shift) -1)<<bit_shift) : 100*PAGE_SIZE);
  ...
  for(int i = 0; i<max_pages_searched; i++)
  {
    ...
    {
      current_stack_top[trampoline_address] = trampoline_address + trampoline_size;
      return trampoline_address;
    }
                          ...
                          CORE_ADDR return_address)
{
  ...
  CORE_ADDR trampoline
      = allocate_trampoline (gdbarch, insn_addr, TP_MAX_SIZE);
